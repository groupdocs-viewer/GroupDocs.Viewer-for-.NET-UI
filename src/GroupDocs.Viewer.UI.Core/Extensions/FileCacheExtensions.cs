...
namespace GroupDocs.Viewer.UI.Core
{
    public static class FileCacheExtensions
    {
        ...
        public static TEntry GetValue<TEntry>(this IFileCache cache, string cacheKey, string filePath, Func<TEntry> acquire)
        {
            var entry = cache.TryGetValue<TEntry>(cacheKey, filePath);
            if (entry == null)
            {
                entry = acquire();
                cache.Set(cacheKey, filePath, entry);
            }
